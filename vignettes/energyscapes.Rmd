...
These data represents the various elements needed for the analyses for the particular case of the bottlenose dolphin in the Mediterranean Sea: the distribution of body mass in the population, the map of abundance, the map of SST (only useful for fish species), the diet composition, the coordinates of the map and the distribution of beta parameter used in the allometric equation. 

```{r example}
plot(density(weight), main = "Body mass distribution in the population")
plot(density(beta), main = "cost of living")
...
```{r plot, echo = FALSE}
# plot results
ggplot(TURTRU_result$Conso) +
    geom_pointrange(aes(y = Prey_category, x = mean, xmin = L10, xmax = U90)) +
  labs(title = "Estimated biomass consumption for bottenose dolphins in Med",
                x = "Mean consumption (kg)",
                y = "Prey category")

ggplot(TURTRU_result$Energyscape) +
  geom_tile(aes(x = x, y = y, fill = Energyscape_mean))+
  labs(title = "Estimated energyscape for bottenose dolphins in Med (kJ/d)",
                x = "Longitude",
                y = "Latitude")
...
```{r plot2, echo = FALSE}
TURTRU_result2 <- mc_simulation(predator_name = "TURTRU", 
                               predator_group = "Cetacean",
             map_coordinates = map_coords,
